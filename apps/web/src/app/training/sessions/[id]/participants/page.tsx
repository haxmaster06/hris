...
export default function SessionParticipantsRoster() {
  ...
  const gradeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["session-participants", sessionId] });
      toast.success("Participant progress updated");
  ...
  const handleRemove = (id: string) => {
    if (confirm("Are you sure you want to remove this participant from the roster?")) {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black font-sans pb-16">
      <Header
        title="Session Participants Roster"
        subtitle="Enroll team members, take class attendance, and log passing certification scores."
      ...
      <main className="max-w-6xl mx-auto px-6 mt-8 space-y-6">
        ...
        {isParticipantsLoading ? (
          ...
          <div className="text-center py-16 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl shadow-sm">
            ...
            <h3 className="text-md font-bold text-zinc-700 dark:text-zinc-300">Roster is empty</h3>
            <p className="text-sm text-zinc-400 dark:text-zinc-600 mt-1">No employees have been enrolled as participants in this session yet.</p>
          </div>
        ) : (
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Employee ID</th>
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Attendance</th>
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Score</th>
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Result</th>
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Remarks</th>
                  ...
                  <tr key={p.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold
                        ...
                        ${p.attendance_status === "Pending" ? "bg-zinc-100 text-zinc-800 dark:bg-zinc-900 dark:text-zinc-400" : ""}
                      `}>
                        {p.attendance_status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          title="Grade/Attend Participant"
                        >
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl flex flex-col">
            <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <h2 className="text-lg font-bold text-zinc-950 dark:text-zinc-50 flex items-center gap-2">
                <Users className="h-5 w-5" /> Enroll Participant
              </h2>
            ...
            <form onSubmit={handleEnrollSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5">Select Employee</label>
                <select
                  required
                ...
                {nonEnrolledEmployees.length === 0 && (
                  <p className="text-[11px] text-zinc-400 mt-1.5">All active employees are already enrolled in this session.</p>
                )}
              </div>
      ...
      {isGradeOpen && selectedParticipant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl flex flex-col">
            <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <h2 className="text-lg font-bold text-zinc-950 dark:text-zinc-50 flex items-center gap-2">
                <GraduationCap className="h-5 w-5" /> Score & Attendance Evaluation
              </h2>
              <button onClick={() => setIsGradeOpen(false)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 text-sm font-semibold">
                Cancel
              </button>
            </div>

            <div className="px-6 py-3 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
              <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Evaluating Employee</p>
            ...
            <form onSubmit={handleGradeSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5">Attendance Status</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5">Kelulusan (Result)</label>
                  <select
                    value={formResult}
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5">Score (Nilai)</label>
                <input
                  type="number"
                  min={0}
                  max={100}
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5">Remarks & Notes</label>
                <textarea
                  placeholder="e.g. Active participation. Exceeded expectations during test."
                  value={formRemarks}
