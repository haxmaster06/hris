        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            {isDashboard ? (
              ...
              <button
                onClick={() => setLauncherOpen(true)}
                className="h-10 w-10 flex items-center justify-center rounded-xl bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 hover:scale-[1.05] active:scale-[0.95] transition-all duration-200 ease-out cursor-pointer"
                title={t("launcher.openLauncher")}
              >
                <LayoutGrid className="h-5 w-5 text-primary" />
              </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={toggleTheme}
              className="h-10 w-10 flex items-center justify-center rounded-lg border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:scale-[1.05] active:scale-[0.95] transition-all duration-200 ease-out cursor-pointer"
            ...
            >
              ...
              <div className="hidden md:block text-left">
                ...
                <p className="text-[9px] text-zinc-500 capitalize mt-0.5">
                  {t("common.company")}: {companyName || tenantId}
                </p>
              </div>
            </button>
            ...
            <button
              ...
              className="h-10 w-10 flex items-center justify-center rounded-lg border border-red-200/50 dark:border-red-900/30 text-red-650 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 hover:scale-[1.05] active:scale-[0.95] transition-all duration-200 ease-out cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-end p-4 md:p-6">
          ...
          <div className="relative w-80 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-3xl p-6 shadow-2xl z-10 mt-16 animate-page-enter">
            ...
            <div className="flex items-center justify-between mb-5">
              <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">{t("profile.title")}</span>
              <button 
                onClick={() => setProfileOpen(false)}
                className="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-900 text-zinc-500 transition-colors cursor-pointer"
              >
            ...
            <div className="text-center space-y-3 mb-6">
              ...
              <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                {user?.roles?.map((role) => (
                  ...
                  <span className="text-[10px] text-zinc-400">{t("profile.noRoles")}</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4 border-t border-zinc-100 dark:border-zinc-900 pt-4">
              ...
              <div className="flex items-center justify-between text-xs text-zinc-500">
                <span className="flex items-center gap-1"><Building className="h-3.5 w-3.5" /> {t("profile.activeTenant")}</span>
              ...
              <div className="space-y-1.5">
                <button
                  ...
                  className="w-full flex items-center justify-between py-1 px-2 rounded-lg bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-900/40 dark:hover:bg-zinc-900 text-xs font-bold text-zinc-600 dark:text-zinc-400 transition-colors cursor-pointer"
                >
                  <span>{t("profile.activePermissions", { count: user?.permissions?.length || 0 })}</span>
                  <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${permissionsExpanded ? "rotate-180" : ""}`} />
                </button>

                {permissionsExpanded && (
                  <div className="max-h-36 overflow-y-auto space-y-1 p-2 bg-zinc-50/50 dark:bg-zinc-900/20 border border-zinc-100 dark:border-zinc-900 rounded-xl text-[10px] text-zinc-650 dark:text-zinc-400 font-mono">
                    {user?.permissions && user.permissions.length > 0 ? (
                      ...
                      <p className="text-center py-2 text-zinc-500">{t("profile.noPermissions")}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="space-y-2 mt-6">
              {user?.employee_id ? (
                <Link
                  ...
                  onClick={() => setProfileOpen(false)}
                  className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl bg-zinc-900 hover:bg-zinc-855 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 font-bold text-xs shadow-md hover:scale-[1.01] active:scale-[0.99] transition-all duration-200"
                >
                  {t("profile.viewEmployee")}
                </Link>
              ) : (
                <button
                  disabled
                  ...
                  title={t("profile.noLinkedTooltip")}
                >
                  {t("profile.noLinkedEmployee")}
                </button>
              )}

              <button
                onClick={() => { setProfileOpen(false); handleLogout(); }}
                className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl border border-red-200/40 hover:bg-red-50/50 dark:border-red-900/20 dark:hover:bg-red-950/10 text-red-650 dark:text-red-400 font-bold text-xs hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 cursor-pointer"
              >
