...
export default function CertificationDashboard() {
  const t = useTranslations("certification.dashboard");
  const tMaster = useTranslations("certification.master");
  const tEmployee = useTranslations("certification.employee");
  ...
  const addRequirementMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t("toast.mapSuccess"));
    ...
    onError: (err: any) => {
      toast.error(err.response?.data?.message || t("toast.mapFailed"));
    }
  });
  ...
  const navigationCards = [
    {
      title: tMaster("pageTitle"),
      description: tMaster("subtitle"),
      icon: Award,
    ...
    {
      title: tEmployee("pageTitle"),
      description: tEmployee("subtitle"),
      icon: ShieldCheck,
      ...
      <main className="max-w-6xl mx-auto px-6 mt-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-5 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">{t("stats.licenseMaster")}</span>
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-5 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">{t("stats.activeLicenses")}</span>
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-5 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">{t("stats.renewals")}</span>
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-5 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">{t("stats.expired")}</span>
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h2 className="text-lg font-bold text-zinc-950 dark:text-zinc-50">{t("matrix.title")}</h2>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">{t("matrix.subtitle")}</p>
            </div>
            <button
            ...
            >
              <Plus className="h-3.5 w-3.5" /> {t("matrix.mapBtn")}
            </button>
          </div>
              ...
              <table className="w-full border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
                    <th className="px-6 py-3.5 font-semibold text-zinc-500 dark:text-zinc-400">{t("matrix.table.position")}</th>
                    <th className="px-6 py-3.5 font-semibold text-zinc-500 dark:text-zinc-400">{t("matrix.table.certification")}</th>
                    <th className="px-6 py-3.5 font-semibold text-zinc-500 dark:text-zinc-400">{t("matrix.table.issuer")}</th>
                    <th className="px-6 py-3.5 font-semibold text-zinc-500 dark:text-zinc-400">{t("matrix.table.mandatory")}</th>
                    <th className="px-6 py-3.5 font-semibold text-zinc-500 dark:text-zinc-400 text-right">{tCommon("actions")}</th>
                  </tr>
                </thead>
                  ...
                  {requirements.map((req) => (
                    <tr key={req.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        `}>
                          {req.is_mandatory ? t("matrix.mandatoryLabel") : t("matrix.optionalLabel")}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl flex flex-col">
            <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              ...
              <button onClick={() => setIsMatrixOpen(false)} className="text-zinc-400 hover:text-zinc-650 dark:hover:text-zinc-200 text-sm font-semibold">
                {tCommon("cancel")}
              </button>
            </div>
            ...
            <form onSubmit={handleMatrixSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5">{t("modal.position")}</label>
                <select
                  required
